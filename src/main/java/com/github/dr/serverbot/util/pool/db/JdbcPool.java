	...
	protected void heartbeat() {
		if (FREE_CONNECTION.size() > 0) {
			...
			try {
				...
				Log.fatal(e);
				try {
					Connection conn = DriverManager.getConnection("jdbc:mariadb://"+ Config.DB_IP+":"+Config.DB_PORT+"/"+Config.DB_NAME,Config.DB_USER,Config.DB_PASSWD);
					// 关闭自动提交
	            	conn.setAutoCommit(false);
					// 将连接加入连接池中
					FREE_CONNECTION.offer(conn);
				} catch (Exception ee) {
					Log.fatal("SQL-POOL",ee);
					throw new RuntimeException();
				}
			} finally {
				try {
